...
///
/// Unit prices are read from [EconomyConfig.powerUpSinglePrice] first
/// (RC-backed) and fall back to [PowerUpCatalog.singlePrice] when RC
/// has no entry. Discount fractions remain in [PowerUpCatalog] — they
/// describe the mechanical pricing ladder, not a LiveOps lever.
class PackPricing {
  PackPricing._();

  static int _unitPriceFor(String powerUpId) {
    final live = EconomyConfig.powerUpSinglePrice(powerUpId);
    if (live != null) return live;
    final fallback = PowerUpCatalog.singlePrice[powerUpId];
    if (fallback == null) {
  ...
  static int totalPrice({
    ...
    final unit = _unitPriceFor(powerUpId);
  ...
  /// Returns null if [powerUpId] isn't in the RC catalog *and* isn't in
  ...
  static int? totalPriceOrNull({
    ...
    final hasLive = EconomyConfig.powerUpSinglePrice(powerUpId) != null;
    if (!hasLive && !PowerUpCatalog.singlePrice.containsKey(powerUpId)) {
      return null;
    }
